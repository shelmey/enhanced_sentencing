...
```{r disp, fig.height = 3}
felonies %>% 
  filter(!is.na(DISPmax)) %>% 
  group_by(Disposition = DISPmax) %>% 
  summarise(Frequency = n()) %>%
  # arrange(Frequency) %>%
  plot_ly(labels = ~Disposition, 
          values = ~Frequency, 
          type = 'pie', textposition = 'inside', textinfo = 'value+percent') %>%
  layout(
    # title = 'Letters',
...
#   geom_text(aes(label = Frequency, y=Frequency/1.5, x = rev(seq(from = .8, by = .1, length.out = nrow(plotData))))) +
...
```{r histogram, fig.cap="Hover your mouse over a point to see defendant information. Defendants convicted of an offense with a mandatory minimum are represented as triangles."}
...
prisonPlot <- tPP %>% filter(TFelony) %>%
  ...
  ggplot(aes(x = myPRISTOT, y = count, 
             fill=Terror,
             stroke = ifelse(public,.1,ifelse(Richard,.1,0)),
             shape  = TMin,
...
`r sum(fc2$TTerror, na.rm = T)` of these defendants have pleaded guilty to and been sentenced for an offense included in 18 USC § 2332b(g)(5). 80% of them received `r ceiling(quantile(tPP$myPRISTOT[tPP[["Full legal name"]] %in% fc2[["Full legal name"]][fc2$TTerror]], probs = .8, na.rm =T))` months or less.  `r sum(tPP$myPRISTOT[tPP[["Full legal name"]] %in% fc2[["Full legal name"]][fc2$TTerror]]==0)` of them received no prison time.
```{r tdisp, fig.height=2.5, fig.cap="Hover your mouse over a point to see defendant information."}
if(!public){
TerrorPlot <- fc2 %>% filter(TTerror) %>%
  ...
  ggplot(aes(x = myPRISTOT, y = count, 
             ...
             text = paste0(`Full legal name`, "\nMonths: ", myPRISTOT, "\nOffenses: ", ttitles, "\n", Court),
             group  = TMin)) +
  labs(x = "Total Prison Time (Months)", y = "", fill = "Mandatory Minimum") +
  geom_point(size = 1) +
  theme_minimal() +
  theme(plot.background = element_blank(),
    panel.grid.minor = element_blank(),
...
TerrorPlot <- TerrorPlot + theme(panel.background = element_rect(fill = 'black'),
         plot.background = element_rect(fill = 'black'),
         axis.title = element_text(colour = "white"),
         legend.text = element_text(colour = "white"),
         legend.title = element_text(colour = "white"))

TerrorPlot %>% ggplotly(tooltip = "text") %>% 
  # config(displayModeBar = F) %>%
  layout(legend = list(x = 0.6, y = .5))
...
myTable <- fc2 %>% filter(TTerror) %>%
  ...
  if(public)myTable <- myTable  %>% select(-`Richard Plea Offense`) 
myTable %>%
